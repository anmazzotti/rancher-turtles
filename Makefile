...
deploy: docker-build manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	kind load docker-image ghcr.io/rancher-sandbox/rancher-turtles-amd64:dev
	cd config/manager && $(KUSTOMIZE) edit set image controller=ghcr.io/rancher-sandbox/rancher-turtles-amd64:dev
	$(KUSTOMIZE) build config/default --stack-trace | kubectl apply -f -
